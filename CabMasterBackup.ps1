...
# powershell.exe -executionpolicy bypass -file "CabMasterBackup.ps1"
#
###

######################## VARIABLES ########################
# Define base directories to search in
$searchDirs = @("C:\AllMasterSoftware")
$folderPatterns = "CabMasterPro*"
...
function Backup-Version {
    ...
    $itemsToBackup = @(
        "*",
        "Bitmap", "Data", "Doc", "Library", "Report",
        "Table", "Template", "ToolData", "Utils"
    )
    ...
    $global:copyTotal += $fileCount

    # Phase 2: Perform copy with progress
    foreach ($file in $localFiles) {
        ...
        Write-Progress -Activity "Backing up versions" `
                       -Status "Copying: $($file.Name)" `
                       -PercentComplete $percent `
                       -CurrentOperation "$global:copyCurrent of $global:copyTotal files" `
...
if ($choice -eq 'A' -or $choice -eq 'a') {
    ...
    Write-Host  -Activity "`nAll versions backed up to: $zipPath" -ForegroundColor Green -Completed
    ...
    $zipPath = Join-Path $backupFolder $zipName
    Compress-Archive -Path "$tempStaging\*" -DestinationPath $zipPath -Force
    Write-Host  -Activity "`nBackup complete: $zipPath" -ForegroundColor Green -Completed
} else {
